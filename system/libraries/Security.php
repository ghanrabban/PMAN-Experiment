...
class CI_Security{
    ...
    public static function check($name, $action = 1, $controller = ""){
        if (empty($controller)) $controller = wbCache::getCached('current', 'controller');

        if (empty($controller)) throw new Exception("Unknown Module");

        if (!self::isPermissionExist($name)) throw new Exception('Unknown Permission Name '.$name);
        ...
        $stmt = $dbconn->executeQuery($query, array(wbUser::getId()));

        while ($role_id = $stmt->fetchColumn()) {
            // check ACCESS
            $query = "SELECT COUNT(1) FROM ".$prefix."_role_permission as a, ".$prefix."_permission as b
                        WHERE a.role_id = ? AND 
                              ...
                              b.permission_name = ?";
                              //AND b.permission_module = ?";

            $count = $dbconn->fetchColumn($query, array($role_id, $action, $name));
        ...
        throw new Exception("Anda tidak memiliki hak akses untuk melakukan operasi ini<br/><br/>Nama Akses : ".self::$accessList[$action]." on ".$name."<br/>Silahkan hubungi Administrator untuk mendapatkan akses tersebut");
    }
    ...
    public static function isPermissionExist($name){
        ...
        $count = $dbconn->fetchColumn($query, array($name));
    ...
    function confirmAuthKey($modName = NULL, $authIdVarName = 'authid')
    {
        ...
        throw new Exception("<p>Operasi yang anda coba lakukan tidak diperkenankan.</p>Anda mungkin telah menekan tombol Back atau Reload pada browser, mencoba kembali operasi yang tidak boleh diulang, atau cookie tidak diaktifkan pada browser anda");
