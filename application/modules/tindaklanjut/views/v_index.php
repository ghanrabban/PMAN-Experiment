...
<script>
   ...
   $('body').on('change','#id_temuan', function() {
      if($(this).val() != ''){
         var id=$(this).val();
         $.ajax({
            url: '<?= base_url('temuan/SetFasilitas/') ?>' + id,
            type: 'post',
            contentType: false,
            processData: false,
            success: function(r){
   ...
   function Setbtn(id){
         ...
         ajax: {
               ...
               data: function (serc) {
                  return {
                     ...
                     jenis:$('#id_perangkatid'+id).find(':selected').attr('data-id')
                  };
                  alert("error");
               },
               results: function (data) {
      ...
      $('#id'+id).on("change", function (e) {
         ...
         if (/ \(new\)$/.test(e.added.text)) {
            var response = confirm("Ingin Menambahan Jenis Masalah " + e.added.id + "?");
            if (response == true) {
               var formData = new FormData();
            
               var idp= $('#id_perangkatid'+id).find(':selected').attr('data-id');
            
               formData.append('nama_masalah', e.added.id);
               formData.append('parent_id', idp);
               $.ajax({
                  url: '<?=base_url()?>jenis_masalah/SaveDataJenis',
                  type: 'post',
                  data: formData,
                  contentType: false,
                  processData: false,
