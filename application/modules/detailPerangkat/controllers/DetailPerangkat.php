...
class DetailPerangkat extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data["title_des"] = " List Detail Perangkat";
    ...
    function LoadData(){
       ...
       // $acc  = access($data,"VIEW")['access'];
         ...
         $data_res['ms']    = $this->Mod->GetCustome('SELECT a.*,b.nama as jenis FROM master_perangkat_detail a left join jenis_perangkat b on b.id_jenisperangkat = a.id_jenisperangkat Where a.status!= 8 and b.id_unit="'.sess()['unit'].'"')->result_array();
         foreach ( $data_res['ms']  as $key => $value) {
            $data_res['ms'][$key]['status_label'] =lb_st($value['status']);
         }
    ...
    function EditData($id=null){
        if (!empty($id)) {
            $data = $this->m_data->getWhere('master_perangkat_detail',array('idmaster_detail_perangkat' =>$id ))->row_array();
    ...
    public function Delete($id=null){
        ...
        $res_update = $this->m_data->updateData('master_perangkat_detail',array('idmaster_detail_perangkat' =>$id ), $data);
    }
