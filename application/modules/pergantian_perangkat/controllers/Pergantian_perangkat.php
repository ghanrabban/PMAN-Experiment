...
class Pergantian_perangkat extends MX_Controller {
    ...
    function LoadUser(){
        ...
        $res   = $this->Mod->GetCustome("SELECT a.*,b.name_role,b.name_code FROM user a left join role b on b.id = a.type_user WHERE a.type_user in ('2','4','5','6') and unit_kerja='".sess()['unit']."'")->result_array();
        ...
        if (!empty($data)) {
            ...
            if (!empty($_POST['Newitems'])) {
				...
				foreach ($_POST['Newitems'] as $key => $value) {
                    ...
                    $data_before    = upload('upload_before', "./upload/temp", "jpg|png|jpeg", 100000, "");
    ...
    public function EditData($id) {
        ...
        $data = $this->Mod->GetCustome("SELECT * from change_divice where id_change  ='".$id."'")->row_array();
       
          $data['detail'] = $this->Mod->GetCustome("SELECT * from change_divice_detail where id_change  ='".$data['id_change']."'")->result_array();
    ...
    public function UpdateData($id=null)
    {
        ...
        function PrintData($id=null){
            ...
            $sig_orgnik                     = $this->Mod->getWhere('sig_log',
                        array('rel_type'    =>'pergantian_perangkat',
                        'rel_id'            =>$id,
                        'create_by'         => $pergantian['id_organik'] ))->row_array();

            $pergantian['sig_orgnik']       = $sig_orgnik ['file_name'];

            $leader                         = $this->Mod->getWhere('user',array('id' =>$pergantian['id_leader']))->row_array();
            $pergantian['leader']           = $leader['nama'];
            $sig_leader                     = $this->Mod->getWhere('sig_log',
                                                array('rel_type' =>'pergantian_perangkat',
                                                'rel_id'=>$id,
                                                'create_by' => $pergantian['id_leader'] ))->row_array();
            $pergantian['sig_leader']       = $sig_leader ['file_name'];

            $planer                         = $this->Mod->getWhere('user',array('id' =>$pergantian['id_planer']))->row_array();
            $pergantian['planer']           = $planer['nama'];
            $sig_planer                     = $this->Mod->getWhere('sig_log',
                                                array('rel_type' =>'pergantian_perangkat',
                                                'rel_id'=>$id,
                                                'create_by' => $pergantian['id_planer'] ))->row_array();
            $pergantian['sig_planer']       = $sig_planer ['file_name'];
        ...
        function SaveTTD($id=null){
            ...
            $cek = $this->Mod->getWhere('sig_log',array('rel_id' =>$_POST['id'],'rel_type'=>'pergantian_perangkat' ))->row_array();
            if (empty($cek)) {
                $this->db->insert('sig_log',$data);
                $res=['status' => 200,'msg'=>'Data Save'];
            }else{
                $this->Mod->update2('sig_log', array('id_sig' => $cek['id_sig']),$data);
        ...
        function LoadDataJP(){
             ...
             $data=  $this->Mod->GetCustome(" SELECT jp.id_jenisperangkat, jp.nama from perangkat p 
        ...
        function DeleteData($id=null){
            if (!empty($id)) {
                $cek   = $this->Mod->getWhere('change_divice',array('id_change' =>$id))->row_array();
                if($this->Mod->delete('change_divice',array('id_change'=>$id))){
                    $this->Mod->delete('change_divice_detail',array('id_change'=>$id));
                    $response=[
                        'code' => '200',
