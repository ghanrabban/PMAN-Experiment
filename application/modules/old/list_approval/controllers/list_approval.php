...
class List_approval extends CI_Controller {
    ...
    public function LoadTiket($input=null){
         ...
         $data = $this->Mod->GetCustome("SELECT a.*, b.nama_fasilitas, c.nama_terminal FROM tiket a LEFT JOIN fasilitas b ON a.id_fasilitas = b.id_fasilitas LEFT JOIN terminal c ON a.id_lokasi = c.id WHERE (a.status != 8 && a.status != 0 && a.status != 9 ) AND a.id_unit='".sess()['unit']."' ORDER BY a.create_date DESC")->result_array();
