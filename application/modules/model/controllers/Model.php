...
class Model extends CI_Controller {
    ...
    public function index()
    {
        ...
        $data["title"] = "Tipe / Model";
        $data["title_des"] = " Tipe / Model Perangkat";
    ...
    function LoadData($from=null){
       ...
       // $acc  = access($data,"VIEW")['access'];
        ...
        $param=[
            'table'         => 'model' ,
            'pk'            => 'id_perangkat' ,
            'parameter'     => array('status !=' => 8, 'id_perangkat >=' => 0) ,
        ...
        foreach ($res['data'] as $key => $value) {
            ...
            $res['data'][$key]['status_label'] = lb_model($value['status']);
            if (!empty($fasilitas)) {
    ...
    function LoadDataBy(){
        ...
        $id_jenisperangkat  = $_POST['id_jenisperangkat'];
        $data = $this->m_data->getWhere('model ',array('id_jenisperangkat' =>$id_jenisperangkat,'merk_id'=>$id_merk))->result_array();
        echo json_encode($data);             
    }

    function LoadModelSpek(){
        $id_model            = $_POST['id'];
        // model_spec
        $data =$this->Mod->GetCustome("SELECT a.idmaster_detail_perangkat,a.nama as nilai,b.nama as nama FROM model_spec a left join master_perangkat_detail b 
        on b.idmaster_detail_perangkat = a.idmaster_detail_perangkat WHERE id_perangkat='".$id_model."'")->result_array();
        foreach ($data as $key => $value) {
           $data[$key]['nilai'] =(!empty($value['nilai']) ? $value['nilai'] : '' );
        }
        echo json_encode($data);             
    }
    ...
    function ViewDetail($id = null){
        if (!empty($id)) {
         $perangkat  = $this->m_data->getWhere('model ',array('status !=' => 8,'id_perangkat' => $id ))->row_array();
         $merk      = $this->m_data->getWhere('merk ',array('id' =>$perangkat['merk_id'] ))->row_array();
         (!empty($merk) ? $perangkat['merk'] = $merk['nama'] : '');
            $detail = $this->m_data->getWhere('model_spec ',array('id_perangkat' =>$perangkat['id_perangkat'] ))->result_array();
        ...
        if (!empty($id)) {
         ...
         (!empty($merk) ? $perangkat['merk'] = $merk['nama'] : '');
            ...
            // }else{
                ...
                $detail = $this->m_data->getWhere('master_perangkat_detail',array('id_jenisperangkat' =>$perangkat['id_jenisperangkat'] ))->result_array();
                foreach ($detail as $key => $value) {
                   ...
                   $cek = $this->m_data->getWhere('model_spec',array('id_perangkat' =>$perangkat['id_perangkat'],'idmaster_detail_perangkat' => $value['idmaster_detail_perangkat'] ))->num_rows();
                   if ( $cek  == 0 ) {
                ...
                $detail_spek = $this->m_data->getWhere('model_spec',array('id_perangkat' =>$perangkat['id_perangkat'] ))->result_array();
                // echo "<pre>",print_r ($detail_spek),"</pre>";
                foreach ($detail_spek as $key3 => $value3) {
                    $property = $this->m_data->getWhere('master_perangkat_detail ',array('idmaster_detail_perangkat' =>$value3['idmaster_detail_perangkat'] ))->row_array();
                        
                        // $detail[$key][$property['nama']] = $value2['nama'];
                        $perangkat['detail'][$key3]['id_perangkat_detail']       = $value3['id_perangkat_detail'];
                        $perangkat['detail'][$key3]['idmaster_detail_perangkat'] = $value3['idmaster_detail_perangkat'];
                        $perangkat['detail'][$key3]['property']                  = $property['nama'];
                        $perangkat['detail'][$key3]['value']                     = (!empty($value3['nama']) ? $value3['nama']: "" );
                }
        ...
        if (!empty($data)) {
            ...
            if (!empty($_POST['master'])) {
				...
				foreach ($_POST['master'] as $key => $value) {
                    ...
                    $this->db->insert('model_spec',$detail);
				}	
        ...
        if (!empty($data)) {
            ...
            if (!empty($_POST['master'])) {
				...
				foreach ($_POST['master'] as $key => $value) {
                    ...
                    $cek_detail = $this->m_data->getWhere('model_spec',array('idmaster_detail_perangkat' =>$value['idmaster_detail_perangkat'],'id_perangkat' => $id ))->row_array();
                    if (!empty($cek_detail)) {
                        echo "Update  old";
                        $update = $this->Mod->update2('model_spec',array('id_perangkat_detail' => $cek_detail['id_perangkat_detail']),$detail);
                    }else{
                ...
                if (!empty($id_delete)) {
                    ...
                    $result = $this->Mod->deletein('model_spec', array('id_perangkat' => $id), 'idmaster_detail_perangkat',$id_delete);
                }
            ...
            if (!empty($_POST['edit'])) {
				foreach ($_POST['edit'] as $key => $value) {
                    ...
                    $update = $this->Mod->update2('model_spec',array('id_perangkat_detail' =>$value['id_perangkat_detail']),$detail);
				}	
			}

           
            $update = $this->Mod->update2('model',array('id_perangkat ' =>$id),$data);
        }
    }
    ...
    //         'status'        => 8,
    //     ];
    //     $res_update = $this->m_data->updateData('model',array('id_perangkat' =>$id ), $data);
    // }
    ...
    function SinModel(){
        $perangkat  = $this->m_data->getWhere('model',array('status !=' => 8))->result_array();
        foreach ($perangkat as $key => $value) {
          

            $detail = $this->m_data->getWhere('master_perangkat_detail',array('id_jenisperangkat' =>$value['id_jenisperangkat'] ))->result_array();
                foreach ($detail as $key2 => $value2) {
                   ...
                   $cek = $this->m_data->getWhere('model_spec',array('id_perangkat' =>$value['id_perangkat'],'idmaster_detail_perangkat' => $value2['idmaster_detail_perangkat'] ))->num_rows();
                   if ( $cek  == 0 ) {
                        $this->db->insert('model_spec',$spek);
                         echo "<pre>",print_r ($spek),"</pre>";
                   }
